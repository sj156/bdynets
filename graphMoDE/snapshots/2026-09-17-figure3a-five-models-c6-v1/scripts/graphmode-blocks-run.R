#!/usr/bin/env Rscript
# D-056: informational by default, exact authority tokens on all write/run paths.
...
if(command %in% c("status","help") && length(args)<=1L) {
    cat("D-056 proposed full168 vs block42-uniform; both m4/rho4, four1200/600 each.\n",
        "Cap13500s science +900s diagnosis requires plan approval, frozen identity and new registration.\n",
    ...
    source(file.path(graphmode_root,"scripts/tests/graphmode-blocks-run-deterministic.R"))
} else {
    if(!(command=="prepare" && length(args)==4L && args[4L]=="--approved-plan" ||
        ...
        stop("Invalid block command; no launch.",call.=FALSE)
    for(f in c("gmde-helpers.R","gmde-state-update.R",
        ...
        "graphmode_gate_refresh.R","graphmode_refresh_run.R","graphmode_expert_blocks.R","graphmode_blocks_run.R")) source(file.path(graphmode_root,"R",f))
    options(warn=2,width=160);api <- graphmode_blocks_run_context()

...
def main():
    if len(sys.argv) != 3 or sys.argv[2] != '--authorized':
        raise SystemExit('Usage: graphmode-joint-controller.py REGISTRATION.rds --authorized')
    ...
    with (directory / 'controller-request.json').open('x') as handle:
        json.dump({'started': time.time(), 'pid': os.getpid(), 'science_cap': 7200,
                   'diagnostic_cap': 600, 'registration': str(registration)}, handle)
    ...
    try:
        with (directory / 'controller.log').open('xb') as log:
            durations = {}
            for current, cap in [('science', 7200), ('diagnose', 600)]:
                command = [shutil.which('Rscript'), '--vanilla', str(root / 'scripts/graphmode-joint-run.R'),

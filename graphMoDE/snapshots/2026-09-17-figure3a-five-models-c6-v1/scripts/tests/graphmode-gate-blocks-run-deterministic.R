# D-061 fixed-input/mock-process checks. No real simulation/PG/subprocess.
...
local({
    ...
    for(f in c("gmde-helpers.R","gmde-state-update.R",
        ...
        "graphmode_gate_refresh.R","graphmode_refresh_run.R","graphmode_expert_blocks.R","graphmode_blocks_run.R","graphmode_gate_blocks.R","graphmode_gate_blocks_run.R")) sys.source(file.path(graphmode_root,"R",f),kernel)
    ...
    assert <- function(x) if(!isTRUE(x)) stop("D-061 assertion: ",paste(deparse(substitute(x)),collapse=" "),call.=FALSE)
    ...
    fixture <- tempfile("graphmode-gate-blocks-run-fixed-",tmpdir="/private/tmp");dir.create(fixture);fixture <- normalizePath(fixture)
    ...
    identity <- graphmode_gate_blocks_run_identity(graphmode_root);runtime <- graphmode4_pilot_runtime()
    ...
    test("scope, balanced order, fixed policies, independent seed candidates and budget", {
        assert(identical(s$arms,c("full","contrast")) && s$gate_inner_steps==4L && is.null(s$inner_steps))
        assert(s$iterations==1200L && s$warmup==600L && s$thin==1L && s$checkpoint_every==300L)
        assert(s$block_policies$full$block_length==42L && s$block_policies$contrast$block_length==42L)
        assert(s$block_policies$contrast$offset_rule=="uniform" && is.null(s$policy) && s$rho==4L)
        ...
        assert(identical(s$block_policies$full,s$block_policies$contrast) && s$gate_max_ess_steps==1000L)
    ...
    test("audited80+D060 immutable; complete new definitions loaded; unfrozen refusal", {
    ...
    test("legacy/changed registration and arm rejected", {
        ...
        b <- r;b$spec$block_policies$contrast$block_length <- 21L;fails(api$graphmode_refresh_run_validate(b))
        ...
        assert(api$graphmode_refresh_run_job(r,1L,"full")$seed!=api$graphmode_refresh_run_job(r,1L,"contrast")$seed)
    })
    test("quoted new entry and hard timeout without starting a process", {
        ...
        assert(any(grepl("scripts/graphmode-gate-blocks-run.R",captured$args,fixed=TRUE)))
    ...
    for(scheme in c("full","contrast")) test(paste("audited D060 step plus passive wrapper scheme",scheme), {
        pol <- list(gate=graphmode_gate_refresh_policy(4L,.4),block=graphmode_expert_blocks_policy(2L,"uniform"),ess_scheme=scheme)
        a <- tape(graphmode_gate_blocks_sweep(initial,tiny,pol$gate,pol$block,graphmode_gate_blocks_policy(tiny,scheme),TRUE))
        ...
        if(scheme=="full") {
            old <- tape(graphmode_blocks_run_step(initial,tiny,pol))
            assert(identical(old$used,b$used) && identical(old$value$state,b$value$state))
        }
    ...
    for(scheme in c("full","contrast")) test(paste("actual fixed-tape states pass complete result validation scheme",scheme), {
        pol <- list(gate=graphmode_gate_refresh_policy(4L,.4),block=graphmode_expert_blocks_policy(2L,"uniform"),ess_scheme=scheme)
        ...
        tape({
            ...
            result <- list(schema=graphmode_gate_blocks_run_version,job_signature=job$signature,input_signature=input$signature,formal_authorized=FALSE,
                checkpoint=list(schema=graphmode_gate_blocks_run_version,policy=pol,status="completed-not-convergence-certified",
    ...
    test("conditional scan accounting rejects wrong order, omissions, schema and shared-budget tampering", {
        pol <- list(gate=graphmode_gate_refresh_policy(4L,.4),block=graphmode_expert_blocks_policy(2L,"uniform"),ess_scheme="contrast")
        g <- tape(api$graphmode_refresh_run_step(initial,tiny,pol))$value$diagnostic$gate
        graphmode_gate_blocks_run_gate_check(g,tiny,pol)
        mutations <- list(
            function(x) {x$schema <- graphmode_gate_refresh_version;x},
        ...
        for(mutate in mutations) fails(graphmode_gate_blocks_run_gate_check(mutate(g),tiny,pol),"scan evidence")
    })
    ...
    test("one new fixed panel and full dispersed starts paired between arms", {
        ...
        assert(blind$schema==graphmode_gate_blocks_run_version && graphmode_warmup_starts_check(blind$starts,blind$fit,s))
    ...
    old_spec <- graphmode_gate_blocks_run_spec
    ...
    patched(list(graphmode_gate_blocks_run_spec=short_spec,graphmode_gate_blocks_run_step=fixed_step,
        ...
        make <- function(name,chain=1L,arm="contrast") {
        ...
        test("new job guard rejects changed paired input ESS budget and arm policy", {
            bad <- f$job;bad$policy$ess_scheme <- "full"
        ...
        test("per-time outer rejection streak follows changing offsets, not block index", {
            m <- graphmode_gate_blocks_run_mechanism(res$checkpoint$diagnostics,f$record$spec)
        ...
        test("actual conditional ESS shared-budget error retains last complete outer state", {
            ...
            api$graphmode_refresh_run_step <- function(...) {
                calls <<- calls+1L
                if(calls==3L) tape(graphmode_gate_blocks_ess(initial$x,initial$v,initial$Z,limited,
        ...
        test("explicit fixed-input diagnosis retains all original flags and complete worker ESS cost", {
            ...
            assert(identical(names(report$arms),c("full","contrast")))
            ...
            bad <- report;bad$arms$contrast$validity$valid <- TRUE;fails(api$graphmode_refresh_run_report_check(bad,rr,sci))
            ...
            bad <- report;bad$arms$contrast$mechanism[[1]]$longest_time_rejection_outer_sweeps[1,1] <- 99
            ...
            fails(api$graphmode_refresh_run_report_check(bad,rr,sci))
            bad <- report;bad$arms$contrast$mechanism[[1]]$gate_ess$retained_block_updates <- 16L
            fails(api$graphmode_refresh_run_report_check(bad,rr,sci))
            assert(report$arms$full$mechanism[[1]]$gate_ess$retained_block_updates==16L)
            assert(report$arms$contrast$mechanism[[1]]$gate_ess$retained_block_updates==144L)
        })
    ...
    test("all original source identities and RNG state/kind unchanged", {
        assert(identical(identity,graphmode_gate_blocks_run_identity(graphmode_root)))
    ...
    cat("PASS",checks,"D-061 fixed-input/mock-process groups; no real data/PG/MCMC, subprocess or old-result rerun.\n")

#!/usr/bin/env Rscript
# D-066: status by default; separate plan/launch tokens for side effects.
...
if (command %in% c("status", "help") && length(args) <= 1L) {
    cat("D-066 proposed reference/cached contrast ESS; paired seeds/starts, four1200/600 each.\n",
        "BOTH expert block42/m4/rho4. Four independent streams; do not pool arms.\n",
        "Cap16200s science +1800s diagnosis requires plan approval, frozen sources and new registration.\n",
        ...
        "No automatic diagnosis, retry, continuation, selection or formal simulation.\n", sep = "")
} else if (command == "check" && length(args) == 1L) {
    source(file.path(graphmode_root, "scripts/tests/graphmode-gate-cache-run-deterministic.R"))
} else {
    if (!(command == "prepare" && length(args) == 4L && args[4L] == "--approved-plan" ||
        ...
        stop("Invalid cache comparison command; no launch.", call. = FALSE)
    for (f in c("gmde-helpers.R", "gmde-state-update.R",
        ...
        "graphmode_receipt_repair.R", "graphmode_gate_cache_run.R")) source(file.path(graphmode_root, "R", f))
    options(warn = 2, width = 160); api <- graphmode_gate_cache_run_context()

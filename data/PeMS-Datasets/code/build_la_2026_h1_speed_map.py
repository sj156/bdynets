#!/usr/bin/env python3
"""Build an hourly PeMS speed map for the Los Angeles area."""
...
OUTPUT_HTML = MAP / "la_area_freeway_speeds_2026_h1.html"
OUTPUT_JSON = PROCESSED / "la_area_freeway_speeds_2026_h1.json"
OUTPUT_SUMMARY = PROCESSED / "la_area_freeway_speeds_2026_h1_summary.csv"
OUTPUT_STATIONS = PROCESSED / "la_area_stations_2026_h1.csv"
OUTPUT_LINKS = PROCESSED / "la_area_speed_links_2026_h1.csv"

META_PATTERN = re.compile(r"^d(?P<district>\d{2})_text_meta_(?P<date>\d{4}_\d{2}_\d{2})\.txt$")
HOUR_PATTERN = re.compile(r"^d(?P<district>\d{2})_text_station_hour_(?P<year>\d{4})_(?P<month>\d{2})\.txt$")
...
def build_speed_matrix(meta: pd.DataFrame) -> tuple[list[str], np.ndarray]:
    ...
    for path in sorted(RAW.glob("d*_text_station_hour_2026_*.txt")):
        if not HOUR_PATTERN.match(path.name):
            continue
        ...
        for chunk in pd.read_csv(
            ...
            chunk["samples"] = pd.to_numeric(chunk["samples"], errors="coerce").fillna(0)
            chunk = chunk[(chunk["samples"] > 0) & chunk["average_speed"].between(1, 100)].copy()
            if chunk.empty:
                continue
            ...
            time_pos = ((times.astype("int64") - start_ns) // one_hour_ns).astype(int)
    ...
    for (district, freeway, direction), group in meta.groupby(["district", "freeway", "direction"], sort=False):
        ...
        for a, b in zip(records, records[1:]):
            ...
            link = {
                ...
                "values": [None if not np.isfinite(v) else int(round(float(v))) for v in values],
            }
            links.append(link)
...
def build_summary(time_labels: list[str], links: list[dict]) -> list[dict]:
    ...
    for idx, label in enumerate(time_labels):
        values = [link["values"][idx] for link in links if link["values"][idx] is not None]
        rows.append(
            {
                "time": label,
...
<head>
  ...
  <title>LA Area Freeway Speeds 2026 H1</title>
...
<div class="page">
  ...
  <aside>
    ...
    <div class="sub">PeMS Station Hour · Jan-Jun 2026 · D7/D8/D12</div>
    <div class="field">
      <span>Time</span>
      <div class="time-row">
    ...
    <div class="stats">
      <div class="stat"><b id="segmentCount"></b><span>segments</span></div>
...
<div class="legend">
  ...
  <div class="legend-info">
    ...
    <div>Segments: <b id="legendSegments"></b></div>
    <div>Data: <b>Station Hour</b></div>
  </div>
</div>
...
function speedColor(value) {{
  if (value == null || Number.isNaN(value)) return colors.missing;
...
function drawMap() {{
  ...
  for (const link of data.links) {{
    ...
    line._values = link.values;
    line._link = link;
    line.bindPopup(() => `<div class="popup"><b>${{link.freeway}} ${{link.direction}}</b><br>${{link.from_station}} → ${{link.to_station}}<br>${{data.timeLabels[activeTime]}}<br>Speed: ${{link.values[activeTime] ?? "NA"}} mph</div>`);
  }}
  updateTime(false);
}}
...
function updateTime(restyle = true) {{
  ...
  if (restyle) {{
    speedLayer.eachLayer(line => {{
      const value = line._values[activeTime];
...
def main() -> int:
    ...
    initial_index = int((INITIAL_TIME - START) / pd.Timedelta(hours=1))
    ...
    print(f"Hourly time steps: {len(time_labels):,}")

...
METRIC_CONFIG = {
    ...
    "saturation": {
        ...
        "unit": "ratio",
        "description": "5-minute entering vehicles converted to hourly flow divided by link capacity",
        "style_max": 1.0,
    },
}
        ...
        features.append(
            {
                ...
                "properties": {
                    ...
                    "vehicle_minutes": None if not hasattr(row, "vehicle_minutes") else round(float(row.vehicle_minutes), 6),
                    "entry_vehicles": None if not hasattr(row, "entry_vehicles") else round(float(row.entry_vehicles), 6),
                    "metric": metric,
                    "metric_label": cfg["label"],
                    "metric_value": round(value, 6),
                    "density_veh_per_km_lane": round(float(row.density_veh_per_km_lane), 6),
                    "storage_capacity_veh": round(float(row.storage_capacity_veh), 3),
                    "congestion_index": round(float(row.congestion_index), 6),
                    "link_type_name": None if pd.isna(row.link_type_name) else str(row.link_type_name),
                    "length_km": None if pd.isna(row.length) else round(float(row.length), 4),
                    "lanes": None if pd.isna(row.number_of_lanes) else int(row.number_of_lanes),
                    "jam_density": None if pd.isna(row.jam_density) else round(float(row.jam_density), 3),
...
def metric_uses_time_weighted_occupancy(metric: str) -> bool:
    return metric in {"density", "congestion", "saturation"} and TIME_WEIGHTED_OCCUPANCY_FILE.exists()
...
def metric_column(metric: str) -> str:
    ...
    if metric == "saturation":
        return "capacity_ratio_from_xijt"
...
def write_dynamic_leaflet_html(
    ...
    if metric == "congestion":
        ...
        display_metric_name = "饱和度"
        legend_title = "饱和度色阶"
        legend_subtitle = "5分钟进入量折算流率 / 通行能力"
    else:
        display_metric_name = str(cfg["label"])
        legend_title = f"{display_metric_name} 色阶"
        legend_subtitle = str(cfg["unit"])
    color_stops_json = json.dumps(CONGESTION_COLOR_STOPS, separators=(",", ":"))
    if metric in {"congestion", "saturation"}:
        legend_segment_width = 100 / len(CONGESTION_COLOR_STOPS)
        legend_gradient = "linear-gradient(to right, " + ", ".join(
            f"{stop['color']} {idx * legend_segment_width:.4g}% {(idx + 1) * legend_segment_width:.4g}%"
            for idx, stop in enumerate(CONGESTION_COLOR_STOPS)
        ) + ")"
        legend_tick_html = "".join(f"<span>{stop['label']}</span>" for stop in CONGESTION_COLOR_STOPS)
    else:
    ...
    for time_bin, group in flow.groupby("time_bin", sort=True):
        ...
        congested_edges = int((group[threshold_col] >= 1.0).sum())
        congested_edge_ids = set(int(edge_index) for edge_index in group.loc[group[threshold_col] >= 1.0, "edge_index"])
        duration_counter = {
...
<body>
  ...
  <div class="{panel_class}">
    ...
    <div class="mode-tabs" role="group" aria-label="地图视图模式">
      ...
      <button class="mode-button" type="button" data-mode="duration">持续拥堵</button>
    </div>
    ...
    <div class="controls">
      ...
      <div class="readout">
        ...
        <div class="stat"><span class="label">超过 100%</span><span class="value" id="jamLabel"></span></div>
  ...
  <script>
    ...
    const modeDetails = {{
      current: {{
        ...
        legendTicks: {json.dumps([stop["label"] for stop in CONGESTION_COLOR_STOPS], ensure_ascii=False)}
      }},
      delta: {{
        label: "较上一片变化",
        listLabel: "恶化最快",
    ...
    function metricIntensity(value) {{
      ...
      if (ratioMetricKeys.has(metricKey)) return Math.min(1, v);
    ...
    function updateSummary(meta, bin) {{
      ...
      if (viewMode === "duration") {{
        ...
        autoSummary.textContent = `${{timeText}} 当前有 ${{Number(meta.congested_edges).toLocaleString()}} 条路段超过 100%；最长连续拥堵 ${{formatDuration(maxDuration)}}，其中 ${{longEdges.toLocaleString()}} 条已持续 30 分钟以上。`;
        return;
      }}
      autoSummary.textContent = `${{timeText}} 有 ${{Number(meta.active_edges).toLocaleString()}} 条路段出现${{summaryValueLabel}}，其中 ${{Number(meta.congested_edges).toLocaleString()}} 条超过 100%；平均${{displayMetricName}} ${{formatMetric(meta.mean_metric)}}，95 分位 ${{formatMetric(meta.p95_metric)}}。`;
    }}
...
def make_run_dir(args: argparse.Namespace) -> Path:
    ...
    default_name = f"{timestamp}_{args.tile}_{args.metric}_{geometry}_{filtering}_{dynamic}_{animation}_t{args.time_bin}"
...
def write_run_readme(
    ...
    lines = [
        ...
        f"- Time bin for static map: `{args.time_bin}`",
    ...
    lines.extend(
        [
            ...
            "- `saturation` is calculated as 5-minute entering vehicles converted to hourly flow divided by `lane_capacity_in_vhc_per_hour`; it is usually the better main-map metric for period-based flow data.",
            "- This avoids dividing a period-total vehicle count by an instantaneous storage capacity.",
...
def main() -> None:
    ...
    network_html = args.out_dir / f"road_network_leaflet_{args.tile}{geometry_suffix}.html"
    write_leaflet_html(
        network_html,
    ...
    flow_html = args.out_dir / f"{prefix}_leaflet_time_bin_{args.time_bin}_{args.tile}{geometry_suffix}.html"
    write_leaflet_html(
        flow_html,
        f"{metric_config(args.metric)['title']}, Time Bin {args.time_bin}",
        args.tile,
        bounds,
        flow_features(links, coords, args.time_bin, args.metric),
        flow_layer=True,
        metric=args.metric,
    )
    ...
    output_paths = {
        "static coordinate preview": static_png,
        "network HTML": network_html,
        "single time-bin metric HTML": flow_html,
